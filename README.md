...
├── BASLAT.bat               # Windows başlatma dosyası
├── kurulum.bat              # Windows kurulum dosyası
...
**1. Python sürümünü kontrol etmek:**

```bash
python --version
```

**2. Gerekli Python kütüphanelerini yüklemek:**

```bash
pip install -r requirements.txt
```

**3. Alternatif olarak otomatik kurulum dosyası kullanmak:**
...
Uygulamayı başlatmak için aşağıdaki yöntemlerden birini kullanabilirsiniz:

**Yöntem 1 - Python komutu:**

```bash
python main.pyw
```

**Yöntem 2 - Windows batch dosyası:**

```bash
BASLAT.bat
```

BASLAT.bat dosyası sanal ortamı aktif eder ve uygulamayı otomatik olarak başlatır.

## ⚡ Kullanım

1. Uygulamayı başlat
2. Aylık gelir bilgisini gir
3. Harcama kategorisini yaz
4. Harcama tutarını gir
5. **Harcama Ekle** butonuna bas
6. Tüm harcamaları ekledikten sonra **Bütçe Analizi Yap** butonuna bas
7. **Bar Grafik Göster** butonu ile kategori karşılaştırmasını görüntüle
8. **Pasta Grafik Göster** butonu ile harcama dağılımını görüntüle
9. **AI Yorum Al** butonu ile bütçe yorumunu görüntüle
...
En yüksek harcama kategoriniz "Kira" kategorisidir ve bu kategori için 6000.00 TL harcama yapılmıştır.
...
Bar grafik, harcama kategorilerini karşılaştırmak için kullanılır. Bu grafik sayesinde kullanıcı hangi kategoriye daha fazla harcama yaptığını kolayca görebilir.
...
Pasta grafik, her harcama kategorisinin toplam gider içindeki oranını göstermek için kullanılır. Bu grafik sayesinde kullanıcı, toplam harcamasının hangi alanlara dağıldığını yüzdelik olarak görebilir.
...
Bu proje eğitim amaçlıdır. Uygulama tarafından üretilen bütçe yorumları genel değerlendirme niteliğindedir ve profesyonel finansal danışmanlık yerine geçmez.
